...
const Intro = ({ queryParams }) => {
  ...
  const toggleInstallAddons = React.useCallback(() => {
    dispatch({ type: "toggle-checkbox", name: "installAddons" });
  }, []);
    ...
    const onCoreEvent = async ({ event, args }) => {
      switch (event) {
        case "UserAuthenticated": {
          // Install default addons automatically for new signups if user chose to
          if (isSigningUpRef.current) {
            isSigningUpRef.current = false;
            if (state.installAddons) {
              (async () => {
                try {
                  await installDefaultAddons(core);
                } catch (error) {
                  console.error(
                    "[Intro] Error installing default addons:",
                    error,
                  );
                } finally {
                  closeLoaderModal();
  ...
  }, [routeFocused, state.installAddons]);
  return (
        ...
        <div className={styles["form-container"]}>
          ...
          {state.form === SIGNUP_FORM ? (
            <React.Fragment>
              ...
              <Checkbox
                label={
                  "Install recommended addons (movies, series, and subtitles)"
                }
                checked={state.installAddons}
                onChange={toggleInstallAddons}
              />
            </React.Fragment>
          ) : (
